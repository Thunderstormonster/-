...
#define TI_CHR_MAXSIZE	2
#define INT_MAXSIZE_PARM   20
#define INT_MINUS_MAXSIZE   0

namespace IT
{
	enum IDDATATYPE { INT = 1, STR = 2, CHR=3 };			//типы данных идентификаторов
	enum IDTYPE { V = 1, F = 2, P = 3, L = 4, S = 5 };	//типы идентификаторов: переменная, функция, параметр, литерал, стандартная функция,условный оператор
	struct Entry                            //Структура содержимого таблицы идентификаторов
	{
		...
		Entry(const char* id, int idxLT, IDDATATYPE datatype, IDTYPE idtype)//запись идентификатораы
		{
	...
	void SetI(IdTable& idtable, int, int);
}