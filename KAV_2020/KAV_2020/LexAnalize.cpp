		...
		for (int i = 0; i < In::Words::amount; i++) // начинаем просмотр массива слов
		{
			...
			for (int j = 0; j < N_GRAPHS; j++)    // ищем подходящий автомат для слова	
			{
				...
				if (FST::execute(fstex)) // если разбор успешный то 
				{
					...
					case LEX_LIBM:   // если стандартная библиотека
					{
						LibBody++;
						if (LibBody > 2)    // если найдено более 1-ой библиотеки
					...
					case LEX_SRC: // лексема функции степень 
					{
						MyFunc++;
						IT::Entry entryit("src", i, IT::STR, IT::S); // создаём элемент ТИ с занесением всех собранных данных
						IT::Add(Tables.IDtable, entryit);						 // добавляем созданный элемент
						LT::Entry entrylt(graph[j].lexema, InStruct.words[i].line, IT::IsId(Tables.IDtable, InStruct.words[i].word)); // создание
																																		 // элемента таблицы лексем
						LT::Add(Tables.Lextable, entrylt); //занесение в таблицу лексем
					...
					case LEX_ID://идентификтор
					{
						...
						if (IT::IsId(Tables.IDtable, InStruct.words[i].word) == -1) // если данный идентификатор не найден повторно 
						{
							...
							else if (Tables.Lextable.table[i - 1].lexema == LEX_ID_TYPE_I) //если предыдущая лесема была лексемой целого типа данных 
							{
								...
								{
									if (Tables.Lextable.table[i - 2].lexema != LEX_TYPE) //если не было предварительного определения словом let
									{
							...
							else if (Tables.Lextable.table[i - 1].lexema == LEX_ID_TYPE_S || Tables.Lextable.table[i - 1].lexema == LEX_ID_TYPE_C) //если предыдущая лесема была лексемой стокового типа данных
							{
								if (isParam == false) // если идентификатор типа переменная не является параметром
								{
									if (Tables.Lextable.table[i - 2].lexema != LEX_TYPE) //если не было предварительного определения словом let
						...
						{
							if (Tables.Lextable.table[i - 1].lexema == LEX_ID_TYPE_I || Tables.Lextable.table[i - 1].lexema == LEX_ID_TYPE_S || Tables.Lextable.table[i - 1].lexema == LEX_ID_TYPE_C || Tables.Lextable.table[i - 1].lexema == LEX_FUNCTION) // если предыдущим был тип данных или лексема функции
							{
