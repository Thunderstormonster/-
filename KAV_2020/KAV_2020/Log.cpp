...
namespace Log
{
	...
	void writeError(std::ofstream* stream, const Error::ERROR& e)//ошибки
	{
		Error = true;
			...
			*log.stream << std::setw(8) << std::left << i << std::setw(15) << std::left << IDtable.table[i].id << std::setw(14) << std::left;
			...
			if (IDtable.table[i].iddatatype == IT::INT) {
				*log.stream << INTEGER;
				...
				if (IDtable.table[i].iddatatype == IT::CHR) {
					*log.stream << CHAR;
			}
				else {
					*log.stream << STRING;
			...
			else
			{
				if (IDtable.table[i].value.vSTR.len != 0) {
					*log.stream << IDtable.table[i].value.vSTR.str;
					...
					if (IDtable.table[i].value.vCHR.len1 != 0) {
						*log.stream << IDtable.table[i].value.vCHR.str1;
					}
					else {
						*log.stream << "NULL";
					}
			...
			if (IDtable.table[i].idtype == IT::F || IDtable.table[i].idtype == IT::S)
			{
				*log.stream << std::setw(16) << std::left << IDtable.table[i].count_parm;
				for (int j = 0; j < IDtable.table[i].count_parm; j++)
				{
					...
					if (IDtable.table[i].parms[j] == 'c')
						*log.stream << std::setw(4) << std::left << "char";
		...
		for (int i = 0; i < IDtable.size; i++)
		{
			...
			else
			{
				if (IDtable.table[i].value.vSTR.len != 0) {
					...
					if (IDtable.table[i].value.vCHR.len1 != 0) {
						std::cout << IDtable.table[i].value.vCHR.str1;
					}
					else {
						std::cout << "NULL";
					}
			...
			if (IDtable.table[i].idtype == IT::F || IDtable.table[i].idtype == IT::S)
			{
				...
				for (int j = 0; j < IDtable.table[i].count_parm; j++)
				{
					...
					if (IDtable.table[i].parms[j] == 'c')
						std::cout << std::setw(4) << std::left << "char";
