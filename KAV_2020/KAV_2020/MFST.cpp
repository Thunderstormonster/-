...
namespace MFST
{
	...
	MfstState::MfstState(short pposition, MFSTSTVECTOR pst, short pnrulechain) //конструктор (позиция на ленте, стек автомата, номер текущей цепочки и текущего правила)
	{
		lenta_position = pposition; //позиция на ленте
		st = pst; //стек автомата
		nrulechain = pnrulechain;//номер текущего правила и цепочки
	};
	MfstState::MfstState(short pposition, MFSTSTVECTOR pst, short pnrule, short pnrulechain) //конструктор (позиция на ленте, стек автомата,номер текущего правила, номер текущей цепочки)
	{
	...
	Mfst::MfstDiagnosis::MfstDiagnosis(short plenta_position, RC_STEP prc_stepen, short pnrule, short pnrule_chain)//диагностика
	...
	Mfst::Mfst(LA::LEX plex, GRB::Greibach pgrebach)//(результат работы лексического анализатора, грамматика Грейбах)
	{
		...
		lenta_position = 0;
		st.push_back(grebach.stbottomT);//добавляет дно стека..вектора
	...
	Mfst::RC_STEP Mfst::step(const Log::LOG& log) //выполнить шаг автомата
	{
		...
		{
			if (ISNS(st.back())) //извлекаем последний элемент стека..вектора и проверяем нетерминал ли он
			{
				GRB::Rule rule;
				if ((nrule = grebach.getRule(st.back(), rule)) >= 0)//смотрим, если такое правило есть идём дальше, если нет то елсе
				{
	...
	{
		for (int k = chain.size - 1; k >= 0; k--)st.push_back(chain.nt[k]); //к = длинне цепочке-1. заносим цепочку в стек
		return true;
	};
	...
	bool Mfst::savediagnois(RC_STEP prc_stepen)
	{
		bool rc = false;
		short k = 0;
	...
	void Mfst::printrules(const Log::LOG& log)//вывести правила вывода
	{
		*log.stream << endl;
		*log.stream << endl;
